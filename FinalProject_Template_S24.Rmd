...
```{r}
# Brier scores
print(paste("Briar score:", compute_brier(clean_weather$RainTomorrow, predict(lr_model, clean_weather, type = "response"))))

pi <- exp(-1.720027 + 1.576018) / (1 + exp(-1.720027 + 1.576018))
odds <- pi / (1 - pi)
print(paste("Odds:", odds))
```
...
```{r}
# Brier scores
print(paste("Briar score:", compute_brier(clean_weather2$RainTomorrow, predict(myModel, clean_weather2, type = "response"))))
```
...
```{r}
# Brier scores
print(paste("Briar score for Sydney:", compute_brier(SydneyWeather$RainTomorrow, predict(model_sydney, SydneyWeather, type = "response"))))
print(paste("Briar score for Melbourne:", compute_brier(MelbourneWeather$RainTomorrow, predict(model_melbourne, MelbourneWeather, type = "response"))))

pi_sydney <- exp(-1.58290 + 1.62910) / (1 + exp(-1.58290 + 1.62910))
odds_sydney <- pi_sydney / (1 - pi_sydney)
print(paste("Odds for Sydney:", odds_sydney))

pi_melbourne <- exp(-1.50550 + 1.02034) / (1 + exp(-1.50550 + 1.02034))
odds_melbourne <- pi_melbourne / (1 - pi_melbourne)
print(paste("Odds for Melbourne:", odds_melbourne))
```
...
```{r}
# Brier scores
print(paste("Briar score for Sydney:", compute_brier(weather_cleaned_sydney$RainTomorrow, predict(rain_tomorrow_sydney_model, weather_cleaned_sydney, type = "response"))))
print(paste("Briar score for Melbourne:", compute_brier(weather_cleaned_melbourne$RainTomorrow, predict(rain_tomorrow_melbourne_model, weather_cleaned_melbourne, type = "response"))))
```
